...
/* ── TESTIMONIALS ──
   Edit these to add / change testimonials.
   avatar: initials shown if no photo
────────────────────────────────────────────── */
const TESTIMONIALS = [
  {
    quote: "Pablo brought something completely different to our festival. His fire performance with the staff was mesmerising — the crowd was silent. He also ran the safety briefing with total professionalism. We're booking him again.",
    name: "Maria K.",
    role: "Festival Director · Fai Jai Flow Festival, Thailand",
    rating: 5,
    initials: "MK"
  },
  {
    quote: "The contact staff workshop blew my mind. Pablo has this incredible way of teaching — he makes you feel the object as part of your body, not something you're manipulating. I've been spinning for 3 years and learned more in 3 hours than the whole previous year.",
    name: "Jonas W.",
    role: "Flow Arts Practitioner · Berlin",
    rating: 5,
    initials: "JW"
  },
  {
    quote: "We hired Pablo for a corporate team-building day using partner movement. We were skeptical — 40 consultants doing contact improv? But he completely nailed the room, made everyone feel safe, and by the end people were actually crying with laughter and connection. Remarkable facilitator.",
    name: "Sarah M.",
    role: "HR Director · Munich",
    rating: 5,
    initials: "SM"
  },
  {
    quote: "The stage he built for our wellness festival was stunning. He understood immediately what we wanted — something organic, living, connected to the land. He worked three 14-hour days without a complaint. Technically excellent, artistically extraordinary.",
    name: "Lena R.",
    role: "Event Producer · Switzerland",
    rating: 5,
    initials: "LR"
  },
  {
    quote: "I took the spatial awareness workshop as part of my physical theatre training and it's genuinely changed how I relate to stage space. The seven spaces framework is something I use in every rehearsal now. Pablo is a real teacher — clear, generous, precise.",
    name: "Tobias A.",
    role: "Physical Theatre Actor · Vienna",
    rating: 5,
    initials: "TA"
  }
];
...
const LINKS = [
  {
    ico: "📸", title: "Instagram",
    sub: "@echo_between",
    url: "https://instagram.com/echo_between",
    type: "primary"
  },
  {
    ico: "📧", title: "Book via Email",
    sub: "pablotalavera089@gmail.com",
    url: "mailto:pablotalavera089@gmail.com",
    type: "primary"
  },
  {
    ico: "🎭", title: "Workshop Portfolio",
    sub: "7 specialist workshops",
    action: () => gp('workshops'),
    type: "secondary"
  },
  {
    ico: "🔥", title: "Performance Setups",
    sub: "Fire, acro, theatre & more",
    action: () => gp('performance'),
    type: "secondary"
  },
  {
    ico: "🎬", title: "Showreel",
    sub: "Watch performances",
    action: () => gp('showreel'),
    type: "secondary"
  },
  {
    ico: "📦", title: "Media Kit",
    sub: "Press info & technical rider",
    action: () => gp('mediakit'),
    type: "secondary"
  }
];
...
function renderTestimonials() {
  ...
  track.innerHTML = TESTIMONIALS.map(t => `
    <div class="testi-slide">
      <div class="testi-rating">${'★'.repeat(t.rating)}</div>
  ...
  dots.innerHTML = TESTIMONIALS.map((_, i) =>
    `<button class="testi-dot ${i === 0 ? 'on' : ''}" onclick="testiGo(${i})" aria-label="Go to testimonial ${i + 1}"></button>`
  ).join('');
...
function testiGo(idx) {
  testiIdx = ((idx % TESTIMONIALS.length) + TESTIMONIALS.length) % TESTIMONIALS.length;
  const track = document.getElementById('testi-track');
  if (track) track.style.transform = `translateX(-${testiIdx * 100}%)`;
  document.querySelectorAll('.testi-dot').forEach((d, i) =>
    d.classList.toggle('on', i === testiIdx));
}

function testiNext() { clearTimeout(testiTimer); testiGo(testiIdx + 1); startTestiAuto(); }
function testiPrev() { clearTimeout(testiTimer); testiGo(testiIdx - 1); startTestiAuto(); }

function startTestiAuto() {
  clearTimeout(testiTimer);
  testiTimer = setTimeout(() => { testiGo(testiIdx + 1); startTestiAuto(); }, 5000);
}

/* ── BOOKING FORM (localStorage) ── */
const BK_KEY = 'pablo_bookings';
...
function updateBookingCount() {
  ...
  document.querySelectorAll('.bf-actions button:last-child').forEach(btn => {
    btn.textContent = `View saved (${n})`;
  });
}
...
function submitBooking(e) {
  ...
  document.getElementById('bf-confirm-email').textContent = entry.email;
  document.getElementById('bf-success').classList.add('show');
  form.reset();
  updateBookingCount();
  setTimeout(() => document.getElementById('bf-success').classList.remove('show'), 5000);
}

function viewBookings() {
  const overlay = document.getElementById('bookings-modal');
  if (!overlay) return;
  renderBookingsList();
...
function renderBookingsList() {
  const body = document.getElementById('bm-list');
  if (!body) return;
  const all = getBookings();
  if (!all.length) {
    body.innerHTML = '<div class="bm-empty">No saved booking requests yet.</div>';
    return;
  }
  body.innerHTML = all.map(b => `
    <div class="bm-entry">
      <button class="bm-del" onclick="deleteBooking(${b.id})" title="Delete">✕</button>
      <div class="bm-entry-name">${b.name} — ${b.type}</div>
      <div class="bm-entry-meta">${b.email} · ${b.date || 'No date'} · ${b.location || 'No location'} · Saved ${new Date(b.savedAt).toLocaleDateString()}</div>
      ${b.message ? `<div class="bm-entry-msg">${b.message}</div>` : ''}
    </div>`).join('');
}
...
function renderLinks() {
  ...
  list.innerHTML = LINKS.map(l => `
    <${l.url ? `a href="${l.url}" target="${l.url.startsWith('mailto') ? '_self' : '_blank'}" rel="noopener noreferrer"` : 'div'} class="link-card"
      ${l.action ? `onclick="(${l.action.toString()})()"` : ''}>
      <div class="link-card-ico">${l.ico}</div>
      <div class="link-card-text">
        <div class="link-card-title">${l.title}</div>
        <div class="link-card-sub">${l.sub}</div>
      </div>
      <div class="link-card-arrow">→</div>
    </${l.url ? 'a' : 'div'}>`).join('');
}

/* ── PAGE NAV: extended for new pages ── */
/* (gp already defined above — we patch it to support mediakit/links) */
const _gpCore = gp;
gp = function(id) {
  const extraPages = ['mediakit', 'links'];
  // Show/hide all pages
  document.querySelectorAll('.pg').forEach(p => p.classList.remove('on'));
  const pg = document.getElementById('pg-' + id);
  if (pg) pg.classList.add('on');
  // Update pill buttons
  const pillMap = { home:0, workshops:1, performance:2, showreel:3, mediakit:4, links:5 };
  document.querySelectorAll('.pb').forEach(b => b.classList.remove('on'));
  const btns = document.querySelectorAll('.pb');
  const idx = pillMap[id];
  if (btns[idx] !== undefined) btns[idx].classList.add('on');
  window.scrollTo({ top: 0, behavior: 'smooth' });
  setTimeout(() => { observe(); observeCounters(); attachCursor(); }, 120);
};
